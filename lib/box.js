...
function boxScrewHead ({center, screwRadius, screwHeadRadius, screwHeadHeight}) {
	return translate([...center, 0], extrudeRotate({segments: 32}, geom2.fromPoints([
		[0, 0],
		[screwHeadRadius, 0],
		[screwHeadRadius - screwRadius, screwHeadHeight],
		[0, screwHeadHeight]
	])));
}

function boxScrewHeads ({size, radius, screwRadius, screwHeadRadius, screwHeadHeight}) {
	const x = size[0] / 2 - radius;
	const y = size[1] / 2 - radius;
	return union(
		boxScrewHead({center: [x, y], screwRadius, screwHeadRadius, screwHeadHeight}),
		boxScrewHead({center: [x, -y], screwRadius, screwHeadRadius, screwHeadHeight}),
		boxScrewHead({center: [-x, y], screwRadius, screwHeadRadius, screwHeadHeight}),
		boxScrewHead({center: [-x, -y], screwRadius, screwHeadRadius, screwHeadHeight})
	);
...
function boxBase ({size, radius, wall, threadRadius}) {
	const height = size[2];
	size = size.slice(0, 2);
	return subtract(
		...
		extrudeLinear({height}, boxSketchScrew({size, wall, radius, screwRadius: threadRadius}))
	);
}
...
function boxCap ({size, radius, wall, screwRadius, screwHeadRadius, oversize}) {
	...
	return subtract(
		...
		extrudeLinear({height: height + 1}, boxSketchScrew({size, radius, screwRadius: screwRadius})),
		boxScrewHeads({size, radius, screwRadius, screwHeadHeight, screwHeadRadius})
	);
}
