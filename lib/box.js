...
function boxCap ({size, radius, wall, screwRadius, screwHeadRadius, oversize}) {
	...
	return subtract(
		union(
			...
			extrudeLinear({height: height + wall}, boxSketchInner({size, wall, radius, retract: 2 * oversize})),
		),
		translateZ(wall, extrudeLinear({height: height + 1}, boxSketchInner({size, wall, radius, retract: 2 * oversize + wall}))),
